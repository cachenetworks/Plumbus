    ...
    def scan_library(self, library: PlexLibrary, job: PlexScanJob) -> PlexScanJob:
        ...
        try:
            ...
            for payload in rows:
                ...
                if movie is None:
                    movie = Movie(library_id=library.id, rating_key=payload["rating_key"], title=payload["title"])
                    self.db.add(movie)
                    self.db.flush()
                for field in (
                    "title", "original_title", "year", "summary", "tagline", "content_rating",
                    "duration_ms", "studio", "rating", "audience_rating", "edition_title",
                    "poster_key", "art_key",
                ):
                    if field in payload:
