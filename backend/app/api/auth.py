...
from app.services.configuration import DiscordConfiguration, IntegrationConfigurationService
...
def _bootstrap_available(db: Session, discord: DiscordConfiguration | None = None) -> bool:
    discord = discord or IntegrationConfigurationService(db).discord()
    if not discord.initial_superadmin_discord_id:
        return False
...
def bootstrap_status(db: Session = Depends(get_db)) -> dict:
    discord = IntegrationConfigurationService(db).discord()
    return {"available": discord.configured and _bootstrap_available(db, discord)}
...
def discord_bootstrap(db: Session = Depends(get_db)) -> RedirectResponse:
    ...
    if not discord.configured:
        raise HTTPException(503, "Discord OAuth is not configured. Complete /setup first.")
    if not _bootstrap_available(db, discord):
        raise HTTPException(404, "Initial setup is not available")
    return RedirectResponse(_oauth_redirect(db, "bootstrap", discord), status_code=302)
...
def discord_login(db: Session = Depends(get_db)) -> RedirectResponse:
    ...
    if not discord.configured:
        raise HTTPException(503, "Discord OAuth is not configured. Complete /setup first.")
    return RedirectResponse(_oauth_redirect(db, "login", discord), status_code=302)
...
def discord_register(invite_token: str, db: Session = Depends(get_db)) -> RedirectResponse:
    ...
    return RedirectResponse(_oauth_redirect(db, "register", discord, invite.id), status_code=302)
...
def discord_callback(code: str, state: str, request: Request, db: Session = Depends(get_db)) -> Response:
    ...
    if identity_response.status_code != 200:
        db.rollback()
        raise HTTPException(400, "Discord identity lookup failed")
    ...
    if oauth_state.flow == "login":
        if not user:
            db.rollback()
            return RedirectResponse(f"{site.app_url}/login?error=invite_required", status_code=302)
    elif oauth_state.flow == "bootstrap":
        if user:
            db.rollback()
            raise HTTPException(409, "Configured SuperAdmin Discord account already exists")
        if (
            not _bootstrap_available(db, discord)
            or discord_id != discord.initial_superadmin_discord_id
        ):
            db.rollback()
            raise HTTPException(403, "This Discord account is not authorized for initial setup")
        user = User(
            discord_id=discord_id,
            username=identity.get("username") or discord_id,
        ...
        db.add(
            AuditLog(
                ...
                event_metadata={"source": "initial_superadmin_bootstrap"},
            )
        )
    elif oauth_state.flow == "register":
        if user:
            db.rollback()
            return RedirectResponse(f"{site.app_url}/login?error=already_registered", status_code=302)
        invite = db.scalar(
            select(Invitation).where(Invitation.id == oauth_state.invite_id).with_for_update()
        )
        if not invite:
            db.rollback()
            raise HTTPException(410, "Invitation unavailable")
        if (
            invite.revoked_at
        ...
        assigned_role = (
            Role.SUPERADMIN
            if discord.initial_superadmin_discord_id
            and discord_id == discord.initial_superadmin_discord_id
            and _bootstrap_available(db, discord)
            else invite.assigned_role
        )
        user = User(
            discord_id=discord_id,
            username=identity.get("username") or discord_id,
        ...
        db.rollback()
        raise HTTPException(400, "Unknown OAuth flow")
    ...
    response = RedirectResponse(f"{site.app_url}/browse", status_code=302)
    response.set_cookie(
        SESSION_COOKIE,
        raw_session,
        max_age=30 * 24 * 60 * 60,
        httponly=True,
        secure=site.secure_cookies,
        samesite="lax",
        path="/",
    )
    return response
