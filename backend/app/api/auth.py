...
def discord_callback(code: str, state: str, request: Request, db: Session = Depends(get_db)) -> Response:
    ...
    if oauth_state.flow == "login":
        if not user:
            db.rollback()
            return RedirectResponse(f"{settings.APP_URL}/login?error=invite_required", status_code=302)
    elif oauth_state.flow == "register":
        if user:
            db.rollback()
            return RedirectResponse(f"{settings.APP_URL}/login?error=already_registered", status_code=302)
        ...
        if invite.revoked_at or (invite.expires_at and invite.expires_at <= now) or (
            invite.max_uses is not None and invite.use_count >= invite.max_uses
        ):
            db.rollback()
            raise HTTPException(410, "Invitation is no longer valid")
        assigned_role = invite.assigned_role
        if settings.INITIAL_SUPERADMIN_DISCORD_ID and discord_id == settings.INITIAL_SUPERADMIN_DISCORD_ID:
            assigned_role = Role.SUPERADMIN
        user = User(
