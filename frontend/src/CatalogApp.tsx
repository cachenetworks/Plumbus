...
function statusPath(items:MediaItem[]):string{
  return `/api/catalog/status?ids=${items.map(item=>item.id).join(',')}`
}
...
async function enrichCatalogStatus(items:MediaItem[]):Promise<MediaItem[]>{
  ...
  return items.map(item=>{
    ...
    return {
      ...item,
      direct_ready:status.ready,
      qualities:status.qualities?.length?status.qualities:item.qualities,
    }
  })
}
...
function bestQuality(values?:string[]):string|undefined{
  return values?.filter(Boolean).sort((a,b)=>qualityRank(b)-qualityRank(a))[0]
}
...
function PosterCard({item,index=0}:{item:MediaItem;index?:number}){
  const quality=bestQuality(item.qualities)
  return <Link className="catalog-poster-card" to={`/media/${item.id}`} onPointerEnter={()=>prefetchMedia(item.id)} onFocus={()=>prefetchMedia(item.id)}>
    ...
    <div className="poster-title-line">
      ...
      <div className="poster-title-status">
        {item.direct_ready&&<span className="poster-status-chip ready">READY</span>}
        {quality&&<span className="poster-status-chip quality">{quality.toUpperCase()}</span>}
      </div>
    </div>
    <div className="catalog-meta"><span>{item.year||'—'}</span>{item.media_type==='show'&&<span>{item.episode_count||0} EPISODES</span>}</div>
  </Link>
}
...
function CatalogPage({searchOnly=false}:{searchOnly?:boolean}){
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      ...
      try{
        ...
        setLoading(false)
        try{
          const enriched=await enrichCatalogStatus(response.items)
          if(!cancelled)setItems(enriched)
        }catch{/* READY/quality enrichment must never block catalog browsing */}
  ...
  return <div className="page catalog-page">
    <section className={`catalog-hero ${searchOnly?'search-mode':''}`}>
      ...
      <div className="catalog-search"><Search size={20}/><input autoFocus={searchOnly} value={q} onChange={event=>setQ(event.target.value)} placeholder="Search titles…"/>{q&&<button onClick={()=>setQ('')}>CLEAR</button>}</div>
...
function CollectionsPage(){
  ...
  useEffect(()=>{
    ...
    let cancelled=false
    const path=`/api/movies?collection=${encodeURIComponent(selected)}&sort=alphabetical&limit=120`
    const cached=peekCached<{items:MediaItem[]}>(path)
    if(cached){setItems(cached.items);setLoading(false)}else setLoading(true)
    cachedApi<{items:MediaItem[]}>(path,60000).then(async result=>{
      if(cancelled)return
      setItems(result.items);setLoading(false)
      try{const enriched=await enrichCatalogStatus(result.items);if(!cancelled)setItems(enriched)}catch{}
...
function MediaDetailPage(){
  ...
  useEffect(()=>{
    ...
    const cached=peekCached<MediaItem>(path)
    if(cached){setItem(cached);setLoading(false);if(cached.media_type==='show'&&cached.seasons?.length)setSeason(current=>current??cached.seasons![0].season_number??0)}else{setLoading(true);setItem(null)}
    setError('')
    cachedApi<MediaItem>(path,120000).then(async media=>{
      setItem(media)
      if(media.media_type==='show'&&media.seasons?.length)setSeason(current=>current??media.seasons![0].season_number??0)
      setLoading(false)
      try{const [enriched]=await enrichCatalogStatus([media]);setItem(enriched)}catch{}
    }).catch(e=>{setError(e.message);setLoading(false)})
  },[id])

  useEffect(()=>{
    if(!item||item.media_type!=='show'||season===null)return
    const path=`/api/movies/${item.id}/episodes?season_number=${season}`
    const cached=peekCached<{episodes:MediaItem[]}>(path)
    if(cached){setEpisodes(cached.episodes);setEpisodesLoading(false)}else setEpisodesLoading(true)
    let cancelled=false
    cachedApi<{episodes:MediaItem[]}>(path,60000).then(result=>{if(!cancelled)setEpisodes(result.episodes)}).catch(e=>{if(!cancelled)setError(e.message)}).finally(()=>{if(!cancelled)setEpisodesLoading(false)})
    return()=>{cancelled=true}
  },[item?.id,item?.media_type,season])
  ...
  return <div className={`stream-detail ${item.is_anime?'anime-detail':''}`}>
    ...
    <div className="stream-detail-content">
      <div className="terminal-kicker"><Terminal size={14}/> {item.is_anime?'ANIME':'MEDIA'} // {item.library?.server_name||'PLEX'}</div>
      <div className="stream-pills"><span>{item.is_anime?'ANIME':item.media_type.toUpperCase()}</span>{item.year&&<span>{item.year}</span>}{item.rating&&<span>{item.rating}</span>}{item.qualities?.map(quality=><span key={quality}>{quality}</span>)}{ready&&<span className="ready-pill" title="H.264/AVC video with AAC/AAC-LC compatible audio">READY</span>}</div>
      <h1>{item.title}</h1><p>{item.summary}</p><div className="stream-meta">{formatRuntime(item.duration_ms)}{item.genres?.length?` · ${item.genres.slice(0,4).join(' · ')}`:''}</div>
      {playable&&<div className="stream-actions"><Link className="netflix-play" to={`/watch/${item.id}`}><Play fill="currentColor" size={24}/> PLAY</Link><VrChatButton mediaId={item.id} title={item.title}/></div>}
    </div>
    {item.media_type==='show'&&<section className="series-browser"><div className="series-browser-head"><div><h2>EPISODES</h2><span>{item.season_count||0} SEASONS · {item.episode_count||0} EPISODES</span></div><select value={season??0} onChange={event=>setSeason(Number(event.target.value))}>{item.seasons?.map(showSeason=><option key={showSeason.id} value={showSeason.season_number??0}>{showSeason.season_number===0?'Specials':`Season ${showSeason.season_number}`} · {showSeason.episode_count||0} episodes</option>)}</select></div>{episodesLoading&&!episodes.length?<div className="episode-grid">{Array.from({length:6}).map((_,index)=><div className="episode-card skeleton" key={index}/>)}</div>:<div className="episode-grid">{episodes.map(episode=><EpisodeCard episode={episode} key={episode.id}/>)}</div>}</section>}
