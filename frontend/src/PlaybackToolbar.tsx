...
export function VrChatButton({mediaId,title}:{mediaId:number;title?:string}){
  ...
  function close(){setResult(null);setError('');setCopied(false)}

  return <>
    <button className="vrchat-inline-button" disabled={loading} onClick={generate} aria-label={`Generate VRChat link for ${title||'media'}`}>
      <RadioTower size={19}/>{loading?'GENERATING…':'VRCHAT LINK'}
    </button>
    {(result||error)&&<div className="vrchat-modal-backdrop" onClick={close}>
      <div className="vrchat-modal" onClick={event=>event.stopPropagation()}>
        <div className="vrchat-modal-head"><div><span>AVPRO</span> VRCHAT STREAM ROUTE</div><button onClick={close}><X size={20}/></button></div>
        {error?<div className="vrchat-error">{error}</div>:result&&<div className="vrchat-modal-body">
          ...
          <textarea readOnly value={result.vrchat_url} onFocus={event=>event.currentTarget.select()}/>
...
// Kept as a compatibility export for older imports. Playback actions now live
// beside the main Play button instead of floating over the catalog.
export function PlaybackToolbar(){return null}
